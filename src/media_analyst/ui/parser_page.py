...
def render_sidebar() -> dict:
    ...
    with st.sidebar:
        ...
        input_method = st.radio(
            '选择输入方式', ['上传文件', '输入目录'], help='上传本地文件或输入目录路径（递归查找 JSON 文件）'
        )
        ...
        if input_method == '上传文件':
            uploaded_files = st.file_uploader(
                '选择 JSON 文件', type=['json'], accept_multiple_files=True, help='支持同时上传多个文件'
            )
            if uploaded_files:
                # 保存上传的文件到临时位置
                import os
            ...
            if dir_input:
                ...
                if target_dir.exists():
                    if target_dir.is_dir():
                        ...
                        # 显示找到的文件列表（可折叠）
                        if file_paths:
                            with st.expander('📄 文件列表（前20个）', expanded=False):
...
def render_statistics(parsed_data: ParsedData, raw_stats: dict | None = None):
    ...
    with col1:
        if has_duplicates:
            st.metric(
                '帖子数', len(parsed_data.posts), delta=f'- {dedup_stats["duplicate_posts"]}', delta_color='inverse'
            )
        else:
    ...
    with col2:
        if has_duplicates:
            st.metric(
                '评论数',
                len(parsed_data.comments),
                delta=f'- {dedup_stats["duplicate_comments"]}',
                delta_color='inverse',
            )
        else:
...
def render_raw_preview(file_paths: List[str]):
    ...
    with st.expander('🔍 原始 JSON 预览', expanded=False):
        ...
        # 选择要预览的文件
        preview_file = st.selectbox('选择文件预览', file_paths, format_func=format_path)
...
def main():
    """数据解析页面主函数"""
    st.set_page_config(page_title='Media Analyst - 数据解析', page_icon='📊', layout='wide')
