...
def test_douyin_detail_with_pydantic_model(
    ...
    # 7. 断言验证
    assert execution.status == ExecutionStatus.COMPLETED, f'期望 COMPLETED，实际是 {execution.status.value}'
    assert execution.return_code == 0, f'期望返回码 0，实际是 {execution.return_code}'
